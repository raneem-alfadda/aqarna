...
import {
  ArrowRight, ChevronDown, User, LogOut, UploadCloud, Paperclip, AlertCircle,
  FileText, CheckCircle2, Circle, X, Info, Loader2,
} from "lucide-react";
...
function allowFile(f: File) {
  const okType =
    ALLOWED.some((a) =>
      a.startsWith(".") ? f.name.toLowerCase().endsWith(a) : f.type.startsWith(a)
    ) || false;
...
/** =========================================
 *  Page wrapper: يغلّف الكومبوننت الداخلي بـ Suspense
 *  ========================================= */
export default function NewObjectionPage() {
  return (
    <Suspense fallback={<div className="p-6 text-sm">جاري التحميل…</div>}>
      <NewObjectionInner />
    </Suspense>
  );
}
...
/** =========================================
 *  Inner client component: يستخدم useSearchParams بأمان
 *  ========================================= */
function NewObjectionInner() {
  ...
  const [title, setTitle] = React.useState(
    invoiceId ? `اعتراض على الفاتورة ${invoiceId}` : "اعتراض على الرسوم"
  );
  ...
  const templates: Record<string, string[]> = {
    ...
    other: ["لدي اعتراض عام على الرسوم الحالية وأحتاج توضيح البنود بالتفصيل."],
  };

  return (
    <div
      dir="rtl"
      className="min-h-screen bg-gradient-to-b from-emerald-50 via-white to-emerald-50 text-slate-900 flex flex-col relative"
    >
      {/* soft blobs */}
      ...
      <main className="mx-auto max-w-5xl w-full px-4 sm:px-6 lg:px-8 py-8 flex-1 space-y-5">
        ...
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          {invoiceId && (
            <span
              className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs bg-emerald-50 border-emerald-200"
              dir="ltr"
            >
              <FileText className="size-4 text-emerald-700" /> فاتورة:{" "}
              <b className="font-mono">{invoiceId}</b>
            </span>
          )}
        </div>
            ...
            <section className="space-y-4">
              ...
              <Field label="الموضوع">
                <input
                  ...
                  className={inputBase}
                  maxLength={120}
                  placeholder="مثال: اعتراض على احتساب رسوم الخدمات المشتركة"
                />
              ...
              <Field label="سبب الاعتراض">
                <textarea
                  ...
                  placeholder="اشرح السبب مع تفاصيل داعمة (تواريخ، مبالغ، مراسلات…)"
                />
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-600">
                  ...
                  {templates[category].map((t, i) => (
                    <button
                      ...
                      className="rounded-full border px-2 py-0.5 hover:bg-emerald-50"
                    >
                      + {i + 1}
                    </button>
                  ))}
            ...
            <section className="space-y-4">
              <Field label="مرفقات (اختياري)">
                <div
                  ...
                  onDrop={onDrop}
                  className="rounded-xl border-2 border-dashed border-emerald-300/70 bg-emerald-50/40 p-5 text-center"
                >
                  ...
                  <div className="mt-2 text-xs text-slate-500">
                    حتى {MAX_FILES} ملفات • {MAX_MB}MB للملف كحد أقصى
                  </div>
                </div>
                    ...
                    {files.map((f, i) => (
                      <li key={i} className="flex items-center gap-3 rounded-xl border p-2">
                        ...
                        <button
                          ...
                          className="ms-auto rounded-full p-1 hover:bg-slate-100"
                          aria-label="إزالة"
                        >
                          <X className="size-4" />
                        </button>
              ...
              <div className="flex items-center justify-between">
                <button type="button" onClick={() => setStep(1)} className="rounded-xl border px-4 py-2 hover:bg-slate-50">
                  رجوع
                </button>
                <button
                  type="button"
            ...
            <section className="space-y-4">
              <div className="rounded-2xl border bg-emerald-50/60 p-4">
                ...
                <ul className="mt-2 text-sm space-y-1">
                  ...
                  <li>
                    <b>النوع:</b> {labelOf(category)} — <b>الأثر:</b>{" "}
                    {impact === "low" ? "منخفض" : impact === "high" ? "مرتفع" : "متوسط"}
                  </li>
                  {invoiceId && (
                    <li>
                      <b>الفاتورة:</b> <span className="font-mono" dir="ltr">{invoiceId}</span>
                    </li>
                  )}
                  ...
                  <li>
                    <b>المرفقات:</b> {files.length} ملف/ملفات
                  </li>
                </ul>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  type="submit"
                  disabled={submitting}
                  className="rounded-xl bg-emerald-600 px-4 py-2 text-white hover:bg-emerald-700 disabled:opacity-60 inline-flex items-center gap-2"
                >
                  {submitting ? <Loader2 className="size-4 animate-spin" /> : <CheckCircle2 className="size-4" />} إرسال
                  الاعتراض
                </button>
                <button type="button" onClick={() => setStep(2)} className="rounded-xl border px-4 py-2 hover:bg-slate-50">
                  رجوع
                </button>
              </div>
            </section>
