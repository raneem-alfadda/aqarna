...
/* ✅ اجبري الصفحة تكون داينامك (يوقف الـ prerender) */
export const dynamic = "force-dynamic";

/* ===================== Mock: invoices API ===================== */
const INVOICE_DB: Record<
  string,
  {
    amount: number;
    customer: string;
...
function PaymentsInner() {
  ...
  function set<K extends keyof typeof card>(k: K, v: string) {
    setCard((c) => ({ ...c, [k]: v }));
  }

  // رسوم/عمولة حسب الوسيلة
  const feeRate = method === "card" ? 0.018 : method === "apple" ? 0.015 : 0;
    ...
    <div dir="rtl" className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm mb-6">
          {load === "loading" && (
            <div className="flex items-center gap-2 text-sm text-slate-600">
              <Loader2 className="size-4 animate-spin" /> تحميل بيانات الفاتورة…
            </div>
          )}
          {load === "error" && <div className="text-sm text-amber-700">تعذّر العثور على الفاتورة. تأكّدي من الرابط.</div>}
          {load === "ready" && summary && (
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-slate-600 flex flex-col">
                  <span>
                    رقم الفاتورة: <span className="font-mono" dir="ltr">{invoiceId}</span>
                  </span>
                  <span>
                    الفترة: {summary.period} — الاستحقاق: <span dir="ltr">{summary.due}</span>
                  </span>
        ...
        <section className="rounded-2xl border bg-white p-5 shadow-sm space-y-5">
          ...
          {saved && method !== "apple" && (
            ...
            >
              <span className="flex items-center gap-2">
                ...
                استخدام البطاقة المحفوظة ({saved.brand} • {saved.last4})
              </span>
          ...
          <div className="rounded-xl border p-3 text-sm flex items-center justify-between">
            ...
            <div className="text-end">
              ...
              <div className="text-sm">
                الإجمالي: <b dir="ltr">{money(total)}</b>
              </div>
            </div>
          </div>
          ...
          {/* Pay button */}
          <button
            disabled={processing || load !== "ready"}
            onClick={pay}
            className="w-full rounded-xl bg-emerald-600 py-3 text-white hover:bg-emerald-700 disabled:opacity-60"
          >
...
function saveLastCard(numMaskedOrRaw: string) {
  ...
  try {
    localStorage.setItem("last_card", JSON.stringify(data));
  } catch {}
}
